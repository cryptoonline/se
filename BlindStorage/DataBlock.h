...
public:
    ...
    ~DataBlock();
    unsigned char* get();
    void set(fileID_t fid, unsigned char* rawData, uint32_t size);
    void generateKey();
    void setKey(unsigned char* key);
    unsigned char* ENC(unsigned char* plaintext, uint32_t size);
    bool isOccupied();
...
private:
    bool isUsed; /*!< This flag shows whether the block is used. It can be used to encrypt all the blocks that are not used.*/
    ...
    uint32_t blockIndex; /*!< Index of the block, it's incremented each time new datablock object is created. Maximum number of blocks can be 2^32*/
    ...
    unsigned char* DEC(unsigned char* ciphertext, uint32_t size);
